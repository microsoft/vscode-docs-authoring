...
export function isValidEditor(editor: vscode.TextEditor, testSelection: boolean, senderName: string) {
    if (editor === undefined) {
        postError("Please open a document to apply " + senderName + " to.");
        return false;
    }

    if (testSelection && editor.selection.isEmpty) {
        if (senderName === "format bold" || senderName === "format italic" || senderName === "format code") {
            debug("VS Code active editor has valid configuration to apply " + senderName + " to.");
            return true;
        }
        postError("No text selected, cannot apply " + senderName + ".");
        return false;
    }
...
export function insertContentToEditor(editor: vscode.TextEditor, senderName: string, content: string) {
    // tslint:disable-next-line:no-console
    console.log("Adding content to the active editor: " + content);
    ...
    if (editor.selection.isEmpty) {
        ...
        editor.edit((target) => {
            target.insert(position, content);
        });
      }
}
...
export function removeContentFromEditor(editor: vscode.TextEditor, senderName: string, content: string) {
    try {
        ...
        debug(senderName + " removed the content: " + content);
    } catch (error) {
        error("Could not remove content from active editor window:" + error);
    }
}
