...
export function getFilesShowQuickPick(mediaType: MediaType, altText: string, options?: any) {
    ...
    dir.files(folderPath, (err: any, files: any) => {
        ...
        if (isArt) {
            files.filter((file: any) => imageExtensions.includes(path.extname(file.toLowerCase()))).forEach((file: any) => {
                items.push({ label: path.basename(file), description: path.dirname(file) })
            })
        } else {
            files.filter((file: any) => markdownExtensionFilter.includes(path.extname(file.toLowerCase()))).forEach((file: any) => {
                items.push({ label: path.basename(file), description: path.dirname(file) })
            })
        }
            ...
            if (!qpSelection) {
                ...
                if (isArt && altText) {
                    if (altText.length > 250) {
                        ...
                        result = internalLinkBuilder(isArt, path.relative(activeFileDir,
                            path.join(qpSelection.description, qpSelection.label).split("\\").join("\\\\")), altText, languageId)
                    }
                } else if (isArt && altText === "") {
                    result = internalLinkBuilder(isArt, path.relative(activeFileDir,
                        path.join(qpSelection.description, qpSelection.label).split("\\").join("\\\\")), altTextFileName, languageId)
                } else if (!isArt) {
                    result = internalLinkBuilder(isArt, path.relative(activeFileDir,
                        path.join(qpSelection.description, qpSelection.label).split("\\").join("\\\\")), selectedText, languageId)
                }
