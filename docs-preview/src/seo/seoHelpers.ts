...
export function getBreadCrumbPath(basePath: string, filePath: string) {
    ...
    if (breadCrumbs.length !== 0) {
        if (breadCrumbs.length > 2) {
            breadCrumb += ` › ... `;
            breadCrumbs.splice(breadCrumbs.length - 1, breadCrumbs.length)
                .forEach((dir) => {
                    breadCrumb += ` › ${dir} `;
                });
        } else {
...
function getRootBreadCrumbs(basePath, filePath) {
    ...
    if (breadCrumbPath.startsWith("/azure")) {
        breadCrumbPath = breadCrumbPath.replace("/azure", "")
            .replace(".json", ".yml");
    }
    ...
    if (existsSync(breadCrumbPath)) {
        ...
        let breadCrumbs: Array<{ breadCrumb: string[], found: boolean }> = [{ breadCrumb, found: false }];
        TOC.forEach((node) => {
...
function getRootNodes(node: any, filePath: string, breadCrumbs: Array<{ breadCrumb: string[], found: boolean }>, index: number) {
...
function findBreadCrumb(breadCrumbs: Array<{ breadCrumb: string[]; found: boolean; }>, breadCrumb: string[]) {
...
function getChildNodes(node: any, filePath: string, breadCrumbs: Array<{ breadCrumb: string[], found: boolean }>, index: number) {
    if (!breadCrumbs[index]) {
        breadCrumbs.push({ breadCrumb: [], found: false });
    }
    if (node.items && node.items.length > 0) {
