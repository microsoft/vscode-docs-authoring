...
export class PackageError extends Error {
	...
	constructor(
		...
		public pkg: Package,
		public innerError: NodeJS.ErrnoException = null /* lgtm[js/remote-property-injection] */
	) {
		super(message);
	}
}
...
export class PackageManager {
	...
	public constructor(private platformInfo: PlatformInformation, private packageJSON: any) {
	...
	public DownloadPackages(
		...
		return this.GetPackages().then(packages => {
			return util.buildPromiseChain(packages, pkg =>
				maybeDownloadPackage(pkg, logger, status, proxy, strictSSL)
			);
		});
	}
	...
	private GetPackages(): Promise<Package[]> {
		return this.GetAllPackages().then(list => {
...
function downloadPackage(
	...
	return new Promise<any>((resolve, reject) => {
		...
		tmp.file({ prefix: 'package-' }, (err, path, fd, cleanupCallback) => {
			...
			resolve(<any>{ name: path, fd, removeCallback: cleanupCallback });
		});
	}).then(tmpResult => {
		pkg.tmpFile = tmpResult;

		let result = downloadFile(pkg.url, pkg, logger, status, proxy, strictSSL).then(() =>
	...
	return new Promise<void>((resolve, reject) => {
		...
		const request = https.request(options, response => {
			if (response.statusCode === 301 || response.statusCode === 302) {
				...
				return resolve(
					downloadFile(response.headers.location || '', pkg, logger, status, proxy, strictSSL)
				);
			}
	...
	return new Promise<void>((resolve, baseReject) => {
		...
		yauzl.fromFd(pkg.tmpFile.fd, { lazyEntries: true }, (err, zipFile) => {
			...
			zipFile.on('entry', (entry: yauzl.Entry) => {
				...
				if (entry.fileName.endsWith('/')) {
					...
					zipFile.openReadStream(entry, (error, readStream) => {
						...
						mkdirp(path.dirname(absoluteEntryPath), { mode: 0o775 }, absoluteEntryPatherror => {
							if (absoluteEntryPatherror) {
								return reject(
									new PackageError(
										'Error creating directory for zip file entry',
										pkg,
										absoluteEntryPatherror
									)
								);
							}
			...
			zipFile.on('error', zipFileError => {
				reject(new PackageError('Zip File Error:' + zipFileError.code || '', pkg, zipFileError));
			});
		});
	}).then(() => {
		// Clean up temp file
