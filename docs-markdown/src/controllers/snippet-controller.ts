            ...
            case "Scoped Search":
                ...
                dir.subdirs(folderPath, (err: any, subdirs: any) => {
                    ...
                    for (const folders in subdirs) {
                        if (Object.prototype.hasOwnProperty.call(subdirs, folders)) {
                ...
                if (workspace) {
                    if (workspace.workspaceFolders) {
                        ...
                        if (openPublishingRepos) {
                            ...
                            openPublishingRepos.map((repo: { pathToRoot: string; url: string }) => {
                                openPublishingOptions.push({ label: repo.pathToRoot, description: repo.url })
                            })
