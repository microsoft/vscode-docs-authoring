...
export async function collapseRelativeLinksInFolder(uri: Uri) {
  await window.withProgress(
    ...
    async (progress, token) => {
      ...
      const filePaths = await workspace.findFiles(
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (collapsedLinkCount > 0) {
          ++filesUpdated;
          message = `collapsed ${collapsedLinkCount} links in ${basename(
            file.fsPath
          )}`;
        }
      ...
      progress.report({
        increment: 100,
        message: `Collapsed links in ${length} files.`,
      });
    }
  );
}
...
export async function collapseRelativeLinksForEditor(
  ...
  const tempReplacements = findReplacements(
    editor.document,
    content,
    null,
    linkRegex
  ...
  if (tempReplacements) {
    for (let i = 0; i < tempReplacements.length; i++) {
      ...
      if (replacement && existsSync(absolutePath)) {
        const relativePath = relative(directory, absolutePath).replace(
          /\\/g,
          "/"
        );
        if (
          relativePath !== replacement.value &&
          `./${relativePath}` !== replacement.value
        ) {
          replacements.push({
...
export function pickLinkType() {
  ...
  window.showQuickPick(items, opts).then((selection) => {
    ...
    const selectionWithoutIcon = selection.label
      .toLowerCase()
      .split(")")[1]
      .trim();
