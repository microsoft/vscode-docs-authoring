    ...
    if (checkEditor(editor)) {
        ...
        dir.files(folderPath, async (err: any, files: any) => {
            ...
            if (!source) {
                ...
                const sourcePath = path.relative(activeFileDir,
                    path.join(source.description, source.label).split("//").join("//"))
                    .replace(/\\/g, "/")
    ...
    if (wordRange) {
        ...
        dir.files(folderPath, async (err: any, files: any) => {
            ...
            if (!image) {
                ...
                const imagePath = path.relative(activeFileDir,
                    path.join(image.description, image.label).split("//").join("//"))
                    .replace(/\\/g, "/")
