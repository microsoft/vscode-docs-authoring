...
import { DependentOperationEnum, LogicalOperatorEnum, OnFailureEnum, OperationEnum, RuleSetTypeEnum, SiblingsInEnum, StoreResultEnum } from "./audit-enum";
...
export class AuditEntry {
    ...
    public setSuccess(success: boolean, current: string = "", count: number = -777, index: number = -777) {
    ...
    public extractCaptures(groups?: Map<string, string>): AuditEntry {
        ...
        if (!Helpers.strIsNullOrEmpty(this.auditRule.artifact_RegexCaptureName) &&
        ...
        switch (this.auditRule.operationType) {
            ...
            case OperationEnum.Has_MD_Artifact:
                if (this.auditRule.artifactDetails.keys.length > 0) {
                    let s: string = "";
                    this.auditRule.artifactDetails.forEach((value, key) => { s + `${key}: ${value}` + "," });
                    if (s.length > 0)
        ...
        if (this.success) {
            ...
            for (var match of matches) {
                if (this.dictionary.has(match.getGroup("value"))) {
                    this.title = this.title.replace(match.getGroup("0"), this.dictionary.get(match.getGroup("value")));
                }                    
            }            
        }
    ...
    public setSummary(dependentEntries: AuditEntry[]) {
        ...
        this.title = this.title.replace("[S/F]", `[${this.dependentSuccessCount}/${this.dependentTestCount}]`);
    }

    public setSummaryCondition(dependentEntries: AuditEntry[], logicalOperatorType: LogicalOperatorEnum) {
        this.dependentRuleCount = this.auditRule.AllDependents().length;
        this.dependentTestCount = dependentEntries.length;
        this.dependentSuccessCount = dependentEntries.filter(e => e.success).length;
        this.dependentFailureCount = dependentEntries.filter(e => !e.success).length;
...
export class AuditRule {
    ...
    public get onFailureOperation(): OnFailureEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.onFailure))
            tmp = OnFailureEnum[this.onFailure];
        return tmp;
    }
    ...
    public get storeResultType(): StoreResultEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.storeResult))
            tmp = StoreResultEnum[this.storeResult];
        return tmp;
    }
    ...
    public shouldStore(success: boolean): boolean {
        if (success) {
            if (this.storeResultType == StoreResultEnum.OnSuccess || this.storeResultType == StoreResultEnum.OnSuccessOrFailure) {
                return true;
            }
        }

        if (!success) {
            if (this.storeResultType == StoreResultEnum.OnSuccessOrFailure)
                return true;
        }
    ...
    public get operationType(): OperationEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.operation))
            tmp = OperationEnum[this.operation];
        return tmp;
    }
    ...
    public get artifactType(): MarkdownEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.artifact))
            tmp = MarkdownEnum[this.artifact];
        return tmp;
    }
    ...
    public get sibling_ArtifactType(): MarkdownEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.sibling_Artifact))
            tmp = MarkdownEnum[this.sibling_Artifact];
        return tmp;
    }
    ...
    public get siblingsInType(): SiblingsInEnum {
        ...
        if (!Helpers.strIsNullOrEmpty(this.siblingsIn))
            tmp = SiblingsInEnum[this.siblingsIn];
        return tmp;
    }
    ...
    public buildRegex() {
        this.artifactRegex = new RegExp(!Helpers.strIsNullOrEmpty(this.artifact_Text) ? this.artifact_Text : "", "gim");
    }
    ...
    public getDependents(auditRules: AuditRule[]) {
        this.dependents = auditRules.filter(e => this.IsDependent
            (e));        
    }

    public hasArtifactCount(count: number) : boolean{
        if (this.artifact_Count != -777)
            return count == this.artifact_Count;
        else if (this.artifact_Count_RangeMax != -777 && this.artifact_Count_RangeMin != -777 && this.artifact_Count_RangeMax > this.artifact_Count_RangeMin)
            return Helpers.getRange(this.artifact_Count_RangeMin, this.artifact_Count_RangeMax).indexOf(count) >= 0;
        else if (this.artifact_Count_RangeMax != -777)
            return count <= this.artifact_Count_RangeMax;
        else if (this.artifact_Count_RangeMin != -777)
            return count >= this.artifact_Count_RangeMin;
        else {
            console.log(`Cannot evalue count for ${this.title}. No Counts are set for Artifact ${this.artifact}`);
            return false;
        }
    }
    ...
    public atIndex(block: ContentBlock, blocks: ContentBlock[]) : boolean{
        if (this.artifact_Index == -777)
            return true;

        if ((blocks.indexOf(block) == (blocks.length - 1)) && this.artifact_Index == -1)
            return true;
    ...
    public dependentConditionRule(rule: AuditRule) : boolean{
        if ((this.conditions.length > 0 && rule.conditions.length > 0) && this.conditions.indexOf(rule.conditions[0]) >= 0)
            return true;
    ...
    public dependentConditionEntry(entry: AuditEntry): boolean{
        if ((this.conditions.length > 0 && !Helpers.strIsNullOrEmpty(entry.conditionName)) && this.conditions[this.conditions.length - 1] == entry.conditionName)
            return true;
    ...
    public anyDependentCondition(entry: AuditEntry) {
        if (Helpers.intersects(this.conditions, entry.initialConditions))
            return true;
    ...
    public test(contentBlocks: ContentBlock[], filename: string, metadata: Map<string, string>, content: string, blocks: ContentBlock[]): AuditEntry[]{
        try {
            let auditEntries: AuditEntry[] = [];            

            // And/Or Tests Have No condition themselves.
            if (this.logicalOperatorType != LogicalOperatorEnum.None && this.operationType != OperationEnum.GateKeeper) {
                for (var dependent of this.dependents) {
                    auditEntries = auditEntries.concat(dependent.test(contentBlocks, filename, metadata, content, blocks));
                }
                ...
                switch (this.logicalOperatorType) {
                    ...
                    case LogicalOperatorEnum.Or:
                        var successes_or = auditEntries.filter(e => e.success && this.dependentConditionEntry(e));
                ...
                var failedDependents = auditEntries.filter(e => this.dependentConditionEntry(e) && !e.success);
                if (undefined !== logicalTestResult) {
                    ...
                    dependents.filter(e => undefined == e.parent).forEach(e => e.parent = logicalTestResult);

                    var successValues = auditEntries.filter(e => !Helpers.intersects(e.initialConditions, logicalTestResult.initialConditions) || (e.success && e.auditRule.rollUpResult));
                    auditEntries = successValues;

                    if (!logicalTestResult.success)
                        auditEntries = auditEntries.concat(failedDependents);
                    ...
                    case OperationEnum.IncludesFile:
                        {
                            ...
                            var match = ContentMatch.getMatches(content, ContentMatch.includeFile).filter(e => e.groups.get("file") == this.artifact_Text)[0];
                    ...
                    case OperationEnum.Has_MD_ArtifactSiblings:
                        {
                            ...
                            artifacts = contentBlocks.filter(e => e.artifactType == this.artifactType).sort((a, b) => b.start - a.start);
                            
                            this.additionalFilters.forEach((value, key) => {
                                artifacts = artifacts.filter(e => e.groups.has(key) && new RegExp(e.getGroup(key), "gim").test(value));
                            });
                            ...
                            for (let i = 0; i < artifacts.length; i++)
                            {
                                ...
                                var artifactText = this.tryGetArtifactText(thisArtifact, this.artifact_RegexCaptureName);
                                if (is<boolean>(artifactText))
                                    continue;

                                var success = false;

                                if (this.artifactRegex.test(artifactText) && this.atIndex(thisArtifact, artifacts)) {
                                    if (this.artifactDetails.keys.length > 0) {
                                        var detailsFound = true;
                                        this.artifactDetails.forEach((value, key) => {
                                            if (!thisArtifact.groups.has(key) || (thisArtifact.groups.has(key) && !new RegExp(value, "gim").test(thisArtifact.getGroup(key)))) {
                                                detailsFound = false;
                                            }
                                        });
                                ...
                                if (success) {
                                    if (this.operationType == OperationEnum.Has_MD_ArtifactSiblings) {
                                        ...
                                        if (this.siblingsInType == SiblingsInEnum.SameFile)
                                            innerBlocks = blocks;
                                            ...
                                            if (null != parent)
                                                innerBlocks = parent.AllInnerBlocks();
                                        }

                                        innerBlocks = innerBlocks.filter(e => e.artifactType == this.sibling_ArtifactType);
                                        innerBlocks = innerBlocks.sort((a, b) => b.start - a.start);

                                        for (var sibling of innerBlocks)
                                        {
                                            var sibling_artifactText = this.tryGetArtifactText(sibling, this.sibling_artifact_RegexCaptureName);
                                            if (is<boolean>(sibling_artifactText))
                                                continue;
                            ...
                            for (var artifactMatch of matches)
                            {
                                ...
                                if (this.shouldStore(thisAuditEntry.success) && (this.operationType == OperationEnum.Has_MD_Artifact || this.operationType == OperationEnum.Has_MD_ArtifactSiblings))
                                ...
                                for (var dependentRule of this.dependents)
                                {
                                    switch (this.dependentOperationType) {
                                        case DependentOperationEnum.AllChildBlocks:
                                            dependentEntries = dependentEntries.concat(dependentRule.test(artifactMatch.AllInnerBlocks(), filename, metadata, content, blocks));
                                            break;

                                        case DependentOperationEnum.CurrentBlocks:
                                            dependentEntries = dependentEntries.concat(dependentRule.test(contentBlocks, filename, metadata, content, blocks));
                                            break;

                                        case DependentOperationEnum.DirectChilden:
                                            dependentEntries = dependentEntries.concat(dependent.test(artifactMatch.AllInnerBlocks(), filename, metadata, content, blocks));
                                            break;

                                        case DependentOperationEnum.AllBlocks:
                                            dependentEntries = dependentEntries.concat(dependent.test(blocks, filename, metadata, content, blocks));
                                            break;
                                    }
                                }

                                if (this.shouldStore(thisAuditEntry.success))
                                    dependentEntries.filter(e => undefined == e.parent).forEach(e => e.parent = thisAuditEntry); // You are the culprit!!

                                thisAuditEntry.setSummary(dependentEntries);

                                auditEntries = auditEntries.concat(dependentEntries);

                                if (this.operationType == OperationEnum.Has_MD_Artifact_Any)
                                    break;
                            }
                            ...
                            if (matches.length > 0) {
                                ...
                                if (this.operationType == OperationEnum.Has_n_MD_Artifacts) {
                                    ...
                                    var currentOrder = artifacts.map(e => e.text).filter(e => this.artifactRegex.test(e));
                                    ...
                                    for (let i = 0; i < compare_required.length; i++) {
                                        ...
                                        if (!new RegExp(compare_required[i], "gim").test(current)) {
                                            var optionalSuccess = !Helpers.strIsNullOrEmpty(this.artifact_Order_Optional) && new RegExp(compare_optional[optional_index], "gim").test(current);
                                            ...
                                            if ((i + 1) == (compare_required.length - 1))
                                                thisAuditEntry = null;
                                    ...
                                    if (currentOrder.length > 0) {
                                        thisAuditEntry.ExtractCaptures(new Map<string, string>(
                                            [
                ...
                case OperationEnum.ForEach_MD_Artifact:
                {
                    ...
                    // Todo: Child Text (not BlockText) needs set up individually. Headers contain there whole secction. Links, just themselvs, etc.
                    let matches: ContentBlock[] = [];
                    matches = contentBlocks.filter(e => e.artifactType == this.artifactType).sort((a, b) => b.start - a.start);
                    for (let i = 0; i < matches.length; i++)
                    {
                        ...
                        if (!Helpers.strIsNullOrEmpty(this.artifact_Text) && this.artifactRegex.test(artifactText))
                            continue;

                        for (var dependent of this.dependents)
                        {
                            auditEntries = auditEntries.concat(dependent.test(thisArtifact.AllInnerBlocks(), filename, metadata, content, blocks));
                        }
                    }
                }
                break;
                ...
                case OperationEnum.Each_MD_Artifact_ContainsText:
                {
                    ...
                    {
                        var thisArtifact = matches[i];
                        let artifactText = this.tryGetArtifactText(thisArtifact, this.artifact_RegexCaptureName);
                        if (is<boolean>(artifactText))
                            continue;
                        ...
                        thisAuditEntry.fileName = filename;
                        thisAuditEntry.auditRule = this;
                        thisAuditEntry.setSuccess((this.artifactRegex.test(artifactText)), thisArtifact.text).extractCaptures(thisArtifact.groups);
                        ...
                        if (thisAuditEntry.success) {
                            let dependentEntries: AuditEntry[] = [];
                            for (var dependent of this.dependents) {
                                dependentEntries = dependentEntries.concat(dependent.test(contentBlocks, filename, metadata, content, blocks));
                            }
                ...
                case OperationEnum.MetadataField_InAllowedValues:
                {
                    if (null != metadata) {
                        ...
                        if (metadata.has(this.metadataField) && !Helpers.strIsNullOrEmpty(metadata.get(this.metadataField))) {
                            var value = metadata.get(this.metadataField);
                            var allowedValues = this.metadataField_AllowedValues.split(",");
                            thisAuditEntry = new AuditEntry();
                            thisAuditEntry.auditRule = this;
                            thisAuditEntry.fileName = filename;
                            thisAuditEntry.setSuccess(allowedValues.includes(value), value).extractCaptures(metadata);
                        }
                        else {
                            thisAuditEntry = new AuditEntry();
                            thisAuditEntry.auditRule = this;
                ...
                case OperationEnum.MetadataField_Equals:
                {
                    if (null != metadata) {
                        ...
                        if (metadata.has(this.metadataField) && !Helpers.strIsNullOrEmpty(metadata.get(this.metadataField))) {
                            ...
                            thisAuditEntry.auditRule = this;
                            thisAuditEntry.fileName = filename;
                            thisAuditEntry.setSuccess(this.metadataField_ExpectedValue.toLowerCase() == value).extractCaptures(metadata);
                        }
                        else {
                            thisAuditEntry = new AuditEntry();
                            thisAuditEntry.auditRule = this;
                        ...
                        if (undefined !== thisAuditEntry) {
                            ...
                            if (thisAuditEntry.success) {
                                ...
                                if (this.shouldStore(thisAuditEntry.success))
                                    dependentEntries.filter(e => null == e.parent).forEach(e => e.parent = thisAuditEntry);
                ...
                case OperationEnum.MetadataField_ContainsText:
                {
                        if (null != metadata) {
                            ...
                            if (metadata.has(this.metadataField) && !Helpers.strIsNullOrEmpty(metadata.get(this.metadataField))) {
                                ...
                                thisAuditEntry.auditRule = this;
                                thisAuditEntry.fileName = filename;
                                thisAuditEntry.setSuccess(new RegExp(this.metadataField_Text, "gim").test(value)).extractCaptures(metadata);
                            }
                            else {
                                thisAuditEntry = new AuditEntry();
                                thisAuditEntry.auditRule = this;
                            ...
                            if (undefined !== thisAuditEntry) {
                                ...
                                if (thisAuditEntry.success) {
                                    ...
                                    if (this.shouldStore(thisAuditEntry.success))
                                        dependentEntries.filter(e => undefined  == e.parent).forEach(e => e.parent = thisAuditEntry);
                ...
                case OperationEnum.File_FileName_ContainsText:
                {
                    ...
                    switch (this.operationType) {
                        ...
                        case OperationEnum.File_FileName_ContainsText:
                            var fileNameMatch = ContentMatch.getMatches(filename, new RegExp(this.file_fileName_text, "gim"))[0];
                ...
                {
                    for (var dependent of this.dependents.filter(e => e.operationType != OperationEnum.KeyMaster)) {
                        auditEntries = auditEntries.concat(dependent.test(contentBlocks, filename, metadata, content, blocks));
                    }
                    ...
                    if (success) {
                        var keyMasters = this.dependents.filter(e => e.operationType == OperationEnum.KeyMaster);
                ...
                case OperationEnum.KeyMaster:
                {
                    ...
                    if (thisAuditEntry.success || this.onFailureOperation != OnFailureEnum.SummarizeDependents)
                        auditEntries = auditEntries.concat(dependentEntries);
                }
                break;
            }
    ...
    public has_Headers(headers: [[number, string, boolean]], count :number = -777, min :number= -777, max :number= -777, InOrder :boolean= false, Exact :boolean= false) : AuditRule
    {
        if (InOrder)
    ...
    public has_ArtifactsInOrder(type: MarkdownEnum, values: string[], Exact: boolean = false): AuditRule {   
        this.operationType = OperationEnum.Has_MD_ArtifactsInOrder;
        this.artifactType = type;
    ...
    public has_Artifact(type: MarkdownEnum, value: string, regexname: string = "0", index: number = -777): AuditRule {   
        this.operationType = OperationEnum.Has_MD_Artifact;
    ...
    public has_Artifact_Any(type: MarkdownEnum, value: string, regexname: string = "0", index: number = -777): AuditRule{    
    ...
    public has_ArtifactSiblings(primary: MarkdownEnum, sibling: MarkdownEnum, type: SiblingsInEnum, primary_regexname: string = "0", sibling_regexname: string = "0"): AuditRule {   
    ...
    public has_ArtifactDetails(type: MarkdownEnum, details: Map<string, string>, index: number = -777): AuditRule{
    ...
    public has_FilteredArtifact(type: MarkdownEnum, value: string, additionalFilters: Map<string, string>, index: number = -777): AuditRule{
    ...
    public Has_ArtifactCount(type: MarkdownEnum, text: string = "", count: number = -777, min: number = -777, max: number = -777): AuditRule{
    ...
    public static *depthFirstTreeTraversal(root: AuditRule, children: (node: AuditRule) => AuditRule[]): IterableIterator<AuditRule> {
    ...
    public TopParent(): AuditRule {
        if (null != this.dependsOn)
            return this.dependsOn.TopParent();
        else
            return this;
    }

    public LearnFromParents() {
        var parents = this.AllParentRules().filter(e => e.logicalOperatorType != LogicalOperatorEnum.None);
        parents.reverse();
    ...
    public SetUpRule() {
        ...
        if (/%[^%]+%/gim.test(this.title) || /%[^%]+%/gim.test(this.artifact_Text) ||
            this.artifactDetails.forEach((value, key) => {
                if (/%[^%]+%/gim.test(key)) {
