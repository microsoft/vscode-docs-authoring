...
suite("Bookmark Controller", () => {
    ...
    test("insertBookmarkExternal::insertContentToEditor", async () => {
        ...
        let counter = 0
        window.showQuickPick = (items: string[] | Thenable<string[]>) => Promise.resolve(qpSelectionItems[counter++]) as Thenable<any>
    ...
    test("insertBookmarkInternal::insertContentToEditor", async () => {
        ...
        window.showQuickPick = (items: string[] | Thenable<string[]>) => Promise.resolve({ label: "### Third Level Heading\r\n", detail: " " }) as Thenable<any>
