...
import { CancellationTokenSource, CompletionContext, CompletionItem, CompletionItemKind, MarkdownString, Position, Range, SnippetString, window } from "vscode";
import { getTripleColonCompletionItems, tripleColonCompletionItemsProvider } from "../../../helper/triple-colon-extensions";
...
suite("Triple Colon Extension Provider", () => {
    test("Get Extension Completion Provider Items", async () => {
        ...
        const completionList = tripleColonCompletionItemsProvider.provideCompletionItems(editor.document, position, source.token, context);
    ...
    test("Select Image Extension Completion Provider", async () => {
        ...
        completionItem.documentation = new MarkdownString("Use the _Image :::image:::_ extension.", false);
        completionItem.insertText = new SnippetString("image type=\"content\" source=\"{source}\" alt-text=\"{alt-text}\":::");
