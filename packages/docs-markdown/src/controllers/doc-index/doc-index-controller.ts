...
import { AuditRule, AuditEntry } from './audit';
...
export function verify() {
	...
	AuditRule.LoadRules(JSON.parse(readFileSync('./AuditRules.json', 'utf-8')));
	...
	if (!editor) {
		...
		let topic = "";
		if (metadata.has("ms.topic")) {
			topic = metadata["ms.topic"];
		}
		...
		if (topic == "") {
			...
			let rules = AuditRule.Rules.filter(e => e.ruleGroup == "MVC" && e.ruleSet.toLowerCase() == topic.toLowerCase() && e.dependsOn == undefined); 
			if (rules.length == 0) {
