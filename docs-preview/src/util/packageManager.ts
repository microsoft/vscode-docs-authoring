...
import { IPackage, IStatus, PackageError } from "./packages";
...
export class PackageManager {
    ...
    public DownloadPackages(logger: Logger, status: IStatus, proxy: string, strictSSL: boolean): Promise<void> {
    ...
    public InstallPackages(logger: Logger, status: IStatus): Promise<void> {
        return this.GetPackages()
            .then((packages) => {
...
function getBaseInstallPath(pkg: IPackage): string {
...
function getNoopStatus(): IStatus {
    return {
        setDetail: (text) => { /* tslint:disable:no-empty */ },
        setMessage: (text) => {/* tslint:disable:no-empty */ },
    };
}

function maybeDownloadPackage(pkg: IPackage, logger: Logger, status: IStatus, proxy: string, strictSSL: boolean): Promise<void> {
...
function downloadPackage(pkg: IPackage, logger: Logger, status: IStatus, proxy: string, strictSSL: boolean): Promise<void> {
    ...
    return new Promise<tmp.SynchrounousResult>((resolve, reject) => {
        tmp.file({ prefix: "package-" }, (err, filePath, fd, cleanupCallback) => {
            ...
            resolve({ name: filePath, fd, removeCallback: cleanupCallback } as tmp.SynchrounousResult);
        });
    }).then((tmpResult) => {
...
function downloadFile(urlString: string, pkg: IPackage, logger: Logger, status: IStatus, proxy: string, strictSSL: boolean): Promise<void> {
...
function installPackage(pkg: IPackage, logger: Logger, status?: IStatus): Promise<void> {
    ...
    return new Promise<void>((resolve, baseReject) => {
        ...
        yauzl.fromFd(pkg.tmpFile.fd, { lazyEntries: true }, (err, zipFile) => {
            ...
            zipFile.on("entry", (entry: yauzl.Entry) => {
                ...
                if (entry.fileName.endsWith("/")) {
                    ...
                    mkdirp(absoluteEntryPath, { mode: 0o775 }, (error) => {
                        if (error) {
                            return reject(new PackageError("Error creating directory for zip directory entry:" + error.code || "", pkg, error));
                        }
                    ...
                    zipFile.openReadStream(entry, (error, readStream) => {
                        if (error) {
                            return reject(new PackageError("Error reading zip stream", pkg, error));
                        }

                        mkdirp(path.dirname(absoluteEntryPath), { mode: 0o775 }, (mkdirpError) => {
                            if (mkdirpError) {
                                return reject(new PackageError("Error creating directory for zip file entry", pkg, mkdirpError));
                            }
            ...
            zipFile.on("error", (error) => {
                reject(new PackageError("Zip File Error:" + error.code || "", pkg, error));
            });
        });
    }).then(() => {
        // Clean up temp file
...
function doesPackageTestPathExist(pkg: IPackage): Promise<boolean> {
    const testPath = getPackageTestPath(pkg);
    if (testPath) {
...
function getPackageTestPath(pkg: IPackage): string {
