import { launchQuickPick } from "./yaml-controller";
...
export function checkForPreviousEntry(options: boolean) {
  ...
  if (currentLine > 0) {
    for (let i = currentLine; i < totalLines; i--) {
      ...
      if (stopAtParent.text.match(itemsScalar)) {
        if (
          stopAtParent.firstNonWhitespaceCharacterIndex === 2 &&
          startingCursorPosition > 2
        ) {
          itemsIndex = false;
          break;
        }
      ...
      if (i === currentLine && i + 1 !== totalLines) {
        ...
        if (
          lineData.firstNonWhitespaceCharacterIndex > startingCursorPosition
        ) {
          itemsIndex = false;
          break;
        }
  ...
  if (currentLine > 0) {
    ...
    for (i = startPosition; i < totalLines; i--) {
      ...
      if (startingCursorPosition === 0) {
        ...
        if (
          checkChild.firstNonWhitespaceCharacterIndex === startingCursorPosition
        ) {
          nameIndex = true;
          break;
        }
  ...
  if (currentLine - 1 > 0) {
    if (
      ...
      if (
        editor.document.lineAt(currentLine - 1)
          .firstNonWhitespaceCharacterIndex === startingCursorPosition
      ) {
        nameIndex = false;
        itemsIndex = false;
      }
