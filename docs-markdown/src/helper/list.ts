...
export function insertList(editor: vscode.TextEditor, listType: ListType) {
    ...
    let previousOuterNumbered = listObjectModel.previousOuter != null && listType === ListType.Numbered ? listObjectModel.previousOuter.listNumber : 0
    ...
    if (indentCount > 0 && indentCount !== 4) {
        newNumberedLines.push(listType === ListType.Numbered ? " ".repeat(indentCount) + "1. " : " ".repeat(indentCount) + "- ")
        ...
        if (indentCount === 0) {
            ...
            const newlineText = listType === ListType.Numbered ? tabPattern + String.fromCharCode(++previousNestedNumbered) + ". " : tabPattern + "- "
            newNumberedLines.push(newlineText)
        }

        const updatedInnerListed = updateOrderedNumberedList(editor, cursorPosition.line + 1, endInnerListedLine, previousNestedNumbered, tabPattern.length, ListType.Alphabet)

        const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, editor.document.lineCount - 1, previousOuterNumbered, 0, ListType.Numbered)
        ...
        const range: vscode.Range = new vscode.Range(cursorPosition.line, 0, endLine, editor.document.lineAt(endLine).text.length)
...
export function createNumberedListFromText(editor: vscode.TextEditor) {
    ...
    let previousNestedListNumbered = listObjectModel.previousNested != null ? listObjectModel.previousNested.listNumber : startAlphabet.charCodeAt(0) - 1
    let previousNestedListType = listObjectModel.previousNested != null ? listObjectModel.previousNested.listType : ListType.Alphabet
            ...
            case CaseType.IndentType:
                if (indentCount === 0) {
                    ...
                    if (previousNestedListType === ListType.Numbered) {
                        ...
                        numberedListLines.push(tabPattern + String.fromCharCode(++previousNestedListNumbered) + ". " + lineText)
                    } else {
...
export function updateOrderedNumberedList(editor: vscode.TextEditor, startLine: number, endLine: number, currentNumber: number, indentCount: number, listType: ListType) {
    ...
    for (let lineNumber = startLine; lineNumber <= endLine; lineNumber++) {
        ...
        if (lineTextIndentCount === indentCount && (lineTextListType === ListType.Numbered || lineTextListType === ListType.Alphabet)) {
            const newlineText = " ".repeat(indentCount) + (listType === ListType.Numbered ? "1" : "1") + ". " + getTextOfNumberedList(lineText, lineTextListType)
            newNumberedLines.push(newlineText)
        } else {
...
export function updateNumberedList(editor: vscode.TextEditor, startLine: number, endLine: number, currentNumber: number, indent: string, isNested: boolean) {
    let lineNumber = startLine
    const newNumberedLines = []

    // Update only the numbered list which is the indent are the same
    const regex = new RegExp(numberedListWithIndentRegexTemplate.replace("{0}", indent.length.toString()))
...
export function updateNestedNumberedList(editor: vscode.TextEditor, startLine: number, endLine: number, currentLineCode: number, indent: string, listType: ListType) {
    const newNumberedLines = []
    for (let lineNumber = startLine; lineNumber <= endLine; lineNumber++) {
        const lineText = editor.document.lineAt(lineNumber).text
        if (lineText.trim() === "") {
...
export function updateNestedAlphabetList(editor: vscode.TextEditor, startLine: number, endLine: number, currentAlphabet: number, indent: string) {
    let lineNumber = startLine
    const newAlphabetLines = []

    // const numberRegex = new RegExp(numberedListWithIndentRegexTemplate.replace("{0}", indent.length.toString()));
    const alphabetRegex = new RegExp(alphabetListWithIndentRegexTemplate.replace("{0}", indent.length.toString()))
...
export function isBulletedLine(text: string) {
    return (text.trim() !== "" && (text.trim() === "-" || text.startsWith("- ")))
}
 ...
 */
export function getListLineNumber(editor: vscode.TextEditor, line: number, listType: ListType, start?: number) {
...
export function findCurrentNumberedListLastLine(editor: vscode.TextEditor, currentLine: number, indentAdjust?: number) {
    let lastLine = -1
...
export function findCurrentNumberedListPreviousNumber(editor: vscode.TextEditor, currentLine: number, indentAdjust?: number) {
...
export function autolistAlpha(editor: vscode.TextEditor, cursorPosition: vscode.Position, alphabet: number) {
    ...
    const updatedInnerListed = updateOrderedNumberedList(editor, lineNumber + 1, endInnerListedLine, alphabet, indentCount, ListType.Alphabet)
    const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, editor.document.lineCount - 1, previousOuterNumbered, 0, ListType.Numbered)
    ...
    const range: vscode.Range = new vscode.Range(lineNumber, cursorPosition.character, endLine, editor.document.lineAt(endLine).text.length)
...
export function autolistNumbered(editor: vscode.TextEditor, cursorPosition: vscode.Position, numbered: number) {
    // Check numbered block
    const lineNumber = cursorPosition.line
    ...
    const updatedInnerListed = updateOrderedNumberedList(editor, lineNumber + 1, endInnerListedLine, numbered, indentCount, ListType.Numbered)
    const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, editor.document.lineCount - 1, previousOuterNumbered, 0, ListType.Numbered)
    numberedLines = numberedLines.concat(updatedInnerListed).concat(updatedOuterListed)
    const endLine = lineNumber + updatedInnerListed.length + updatedOuterListed.length
    const range: vscode.Range = new vscode.Range(cursorPosition.line, cursorPosition.character, endLine, editor.document.lineAt(endLine).text.length)
...
export function nestedNumberedList(editor: vscode.TextEditor, cursorPosition: vscode.Position, indentCount: number) {
    const previousLine = cursorPosition.line - 1
    const nextLine = cursorPosition.line + 1
    const previousInnerNumbered = getListLineNumber(editor, previousLine, ListType.Numbered, cursorPosition.character)
    const previousaphabet = getListLineNumber(editor, previousLine, ListType.Alphabet, cursorPosition.character)
    const nextnumbered = getListLineNumber(editor, nextLine, ListType.Numbered, cursorPosition.character)
    const nextalphabet = getListLineNumber(editor, nextLine, ListType.Alphabet, cursorPosition.character)
    ...
    if (previousInnerNumbered > 0 || (previousaphabet <= 0 && nextnumbered > 0)) {
        ...
        const updatedInnerListed = updateNestedNumberedList(editor, cursorPosition.line, endInnerListedLine, numbered, tabPattern.repeat(newIndentCount / 4), ListType.Numbered)
        const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, endOuterListedLine, previousOuterNumbered, indentCount, ListType.Numbered)
        ...
        const range: vscode.Range = new vscode.Range(cursorPosition.line, 0, endOuterListedLine, editor.document.lineAt(endOuterListedLine).text.length)
        ...
        const updatedInnerListed = updateNestedNumberedList(editor, cursorPosition.line, endInnerListedLine, alphabet, tabPattern.repeat(newIndentCount / 4), ListType.Alphabet)
        const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, endOuterListedLine, previousOuterNumbered, indentCount, ListType.Numbered)
        const updatedListedText = updatedInnerListed.concat(updatedOuterListed).length > 0 ? updatedInnerListed.concat(updatedOuterListed).join("\n") : ""
        const range: vscode.Range = new vscode.Range(cursorPosition.line, 0, endOuterListedLine, editor.document.lineAt(endOuterListedLine).text.length)
        common.insertContentToEditor(editor, nestedNumberedList.name, updatedListedText, true, range)
        ...
        let previousNestedNumbered = listObjectModel.previousNested != null ? listObjectModel.previousNested.listNumber : startAlphabet.charCodeAt(0) - 1
        let previousNestedListType = listObjectModel.previousNested != null ? listObjectModel.previousNested.listType : ListType.Alphabet
        ...
        const updatedInnerListed = updateNestedNumberedList(editor, cursorPosition.line + 1, endInnerListedLine, previousNestedNumbered, tabPattern.repeat(newIndentCount / 4), previousNestedListType)
...
export function removeNestedListSingleLine(editor: vscode.TextEditor) {
    ...
    const indentCount = CountIndent(editor.document.lineAt(cursorPosition.line).text)
    const numberedRegex = new RegExp(fixedNumberedListWithIndentRegexTemplate.replace("{0}", indentCount.toString()))
    const alphabetRegex = new RegExp(fixedAlphabetListWithIndentRegexTemplate.replace("{0}", indentCount.toString()))
    ...
    if (fixedBulletedListRegex.exec(text) != null) {
        if (indentCount >= 4) {
            ...
            editor.edit(update => {
                update.delete(new vscode.Range(
        ...
        if (indentCount >= 4) {
            ...
            const endInnerListedLine = listObjectModel.nextNested != null ? listObjectModel.nextNested.line : cursorPosition.line
            const endOuterListedLine = listObjectModel.nextOuter != null ? listObjectModel.nextOuter.line : endInnerListedLine
            const updatedListed = updateNumberedList(editor, cursorPosition.line, cursorPosition.line, previousOuterNumbered, tabPattern.repeat((indentCount / 4) - 1), true)
            const updatedInnerListed = updateNestedNumberedList(editor, cursorPosition.line + 1, endInnerListedLine, previousNestedNumbered, tabPattern.repeat(indentCount / 4), previousNestedListType)
            const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, endOuterListedLine, previousOuterNumbered, indentCount - tabPattern.length, ListType.Numbered)
            ...
            editor.edit(update => {
                update.replace(
                    new vscode.Range(cursorPosition.line, 0, endOuterListedLine, editor.document.lineAt(endOuterListedLine).text.length), updatedListedText)
            })
            ...
            const updatedOuterListed = updateOrderedNumberedList(editor, cursorPosition.line + 1, editor.document.lineCount - 1, 0, 0, ListType.Numbered)
        ...
        if (indentCount >= 4) {
            ...
            const endOuterListedLine = listObjectModel.nextOuter != null ? listObjectModel.nextOuter.line : endInnerListedLine
            const updatedListed = updateNumberedList(editor, cursorPosition.line, cursorPosition.line, previousOuterNumbered, tabPattern.repeat((indentCount / 4) - 1), true)
            const updatedInnerListed = updateNestedNumberedList(editor, cursorPosition.line + 1, endInnerListedLine, startAlphabet.charCodeAt(0) - 1, tabPattern.repeat(indentCount / 4), ListType.Alphabet)
            const updatedOuterListed = updateOrderedNumberedList(editor, endInnerListedLine + 1, endOuterListedLine, previousOuterNumbered, indentCount - 4, ListType.Numbered)
            const updatedListedText = updatedListed.concat(updatedInnerListed).concat(updatedOuterListed).length > 0 ? updatedListed.concat(updatedInnerListed).concat(updatedOuterListed).join("\n") : ""
            editor.edit(update => {
                update.replace(
                    new vscode.Range(cursorPosition.line, 0,
                        endOuterListedLine, editor.document.lineAt(endOuterListedLine).text.length),
                    updatedListedText)
            })
...
export function createLineObjectModel(editor: vscode.TextEditor, line: number) {
    ...
    const listType = getListTypeOfNumberedList(lineText)
    const listNumber = listType === ListType.Numbered ? +getNumberTextOfNumberedList(lineText, listType) : getNumberTextOfNumberedList(lineText, listType).charCodeAt(0)
        ...
        switch (listType) {
            case ListType.Bulleted:
                if (lineCount > 0 || lineListType !== ListType.Bulleted) {
                    ...
                    if ((lineCount === 0 && lineListType !== ListType.Bulleted) || lineCount !== tabPattern.length || lineListType !== ListType.Bulleted) {
            ...
            case ListType.Numbered:
                if (lineCount > 0 || lineListType !== ListType.Numbered) {
                    ...
                    if ((lineCount === 0 && lineListType !== ListType.Numbered) || lineCount !== tabPattern.length || (lineListType !== ListType.Numbered && lineListType !== ListType.Alphabet)) {
                        isUnIndentNestedType = false
                        line = endSelectedLine + 1
                    }
                }
