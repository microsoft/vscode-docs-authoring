...
import { execPromise, metadataDirectory, getExtensionPath, output } from '../util/common';
import { openFolderInExplorerOrFinder } from '../util/PlatformInformation';

export async function showApplyMetadataMessage(path: string) {
	if (path.indexOf('.csv') === -1) {
		window
			.showInformationMessage(
				`Please find a Metadata Update Tool file to apply...`,
				'Find MUT file',
				'Cancel'
			)
		...
		window
			.showInformationMessage(
				`Apply Metadata Update Tool(MUT) file ${path}?`,
				'OK',
				'Change Location',
				'Cancel'
			)
...
async function applyMetadata(filePath: string) {
	...
	}mdapplycore.dll" "${filePath}" --nobackup -l "${logFilePath}"`;
	await execPromise(command)
		.then(async result => {
			window
				.showInformationMessage(
					`Metadata apply completed. Check the "docs-metadata" output channel for details.`,
					'Open Folder'
				)
				.then(async selectedItem => {
		...
		.catch(result => {
			if (result.stderr.indexOf(`'dotnet' is not recognized`) > -1) {
				...
				window.showErrorMessage(
					`Couldn't apply metadata. Please make sure the csv file is not in use by another program, and try again.`
				);
			} else {
